...
void main()
{
  ...
  for (i=0; i<1000; i++)
  {
    ...
    //for(m=0; m<N_part; m++){printf( "iter--->%d; force---->%e \n",i, sqrt(pow2(particles[m].forcex) + pow2(particles[m].forcey) + pow2(particles[m].forcez)));}
    //printf( "distance---->%e \n", sqrt(pow2(particles[0].x-particles[1].x) + pow2(particles[0].y-particles[1].y) + pow2(particles[0].z-particles[1].z)));
    //printf("vel tierra---->%e\n", sqrt(pow2(particles[1].vx) + pow2(particles[1].vy) + pow2(particles[1].vz)));
...
Particle        *particles_creator(int N_part, int N_gridpoints, double H)
{
  ...
  /*
  particles[0].x=10.10*H;   //Sun
  particles[0].y=10.10*H;
  particles[0].z=10.10*H;
  particles[1].x=11.76667*H;//Earth
  particles[1].y=10.10*H;
  particles[1].z=10.10*H;
  particles[2].x=12.6326*H;//Mars
  particles[2].y=10.10*H;
  particles[2].z=10.10*H;
  particles[3].x=18.7490*H;//Jupiter
  particles[3].y=10.10*H;
  particles[3].z=10.10*H;

  particles[0].mass=2.0e30;
  particles[1].mass=5.9736e24;
  particles[2].mass=6.4185e23;
  particles[3].mass=1.899e27;

  particles[0].vx = 0.0;
  particles[0].vy = 0.0;
  particles[0].vz = 0.0;
  particles[1].vx = 0.0;
  particles[1].vy = 29650.0;
  particles[1].vz = 0.0;
  particles[2].vx = 0.0;
  particles[2].vy = 24077;
  particles[2].vz = 0.0;
  particles[3].vx = 0.0;
  particles[3].vy = 13069.7;
  particles[3].vz = 0.0;
*/
   ...
   for (i=0; i<N_part; i++)
   {
   particles[i].x = 7*H+ (double)rand()/(double)RAND_MAX*(N_gridpoints-7)*H;
   particles[i].y = 7*H+ (double)rand()/(double)RAND_MAX*(N_gridpoints-7)*H;
   particles[i].z = 7*H+ (double)rand()/(double)RAND_MAX*(N_gridpoints-7)*H; 

   particles[i].vx = (double)rand()/(double)RAND_MAX*40000;
   particles[i].vy = (double)rand()/(double)RAND_MAX*50000;
   particles[i].vz = (double)rand()/(double)RAND_MAX*40000; 
   //particles[i].mass=5.9736e28;
   if (i==49){
     particles[i].mass=6.9736e31;particles[i].x = 20*H; particles[i].y = 20*H;particles[i].z = 20*H;
   particles[i].vx = 0.0;
   particles[i].vy = 0.0;
   particles[i].vz = 0.0;}
   else{particles[i].mass=9.1736e27;}
   }
...
void Potential_GridPoint(Grid_Point ***grid, int N_gridpoints, double H)
{
  ...
  //start iterations
  for(iterations=0; iterations<150; iterations++)
  {
    for(i=0; i<=(N_gridpoints-1); i++) //for the "red" grid points
    {
      for(j=0; j<=(N_gridpoints-1); j++)
